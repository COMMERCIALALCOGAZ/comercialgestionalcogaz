...
const wating_page = document.getElementById('wating_page');
...
auth.onAuthStateChanged(async (user) => {
 if (user) {
   wating_page.style.display="none";
...
function ouvrirlemenu(userid){
 ...
 if (width > 1000) {
     controlbg.style.width = "35%";
     datashowbg.style.width = "65%";
 } else if (width > 800) {
     controlbg.style.width = "30%";
     datashowbg.style.width = "70%";
 } else if (width > 500) {
     controlbg.style.width = "20%";
     datashowbg.style.width = "80%";
 } else {
...
`;
   ...
   async function actualiserdraft(){
     wating_page.style.display="flex";
     ...
     const draftCollectionQuery_ = await getDocs(
      ...
      if (draftCollectionQuery_.empty) {
       wating_page.style.display="none";
     }
   ...
   async function actualisermessageSent(){
     ...
     const exporterMessageSentactualiserbtn = document.createElement("button");
     exporterMessageSentactualiserbtn.classList.add("btnmodifier");
     exporterMessageSentactualiserbtn.textContent = "Imprimer";
     dataShowBg.appendChild(exporterMessageSentactualiserbtn)     
     ...
     let nam_data_for_telecharger = [] ;
     let somme_data_for_telecharger = [] ;
     let sommeLogiciel_data_for_telecharger = [] ;
     let time_data_for_telecharger = [] ;
     let nam_reseption_data_for_telecharger = [] ;
     let sommeCaisse_data_for_telecharger = [] ;
     let duredefiltrage = "";
     let list_diff_log_encaisser = [] ;


     draftCollectionQuery_.forEach(async (doc) => {
          const data = doc.data();
          const borderauid = doc.id;
          const archiveId = data.archiveId;
          const timestampMillis = data.timestamp; // Supposons que le timestamp soit en millisecondes
      ...
      // Display arrays in the console
      console.log("Liste des noms:", nam_data_for_telecharger);
      console.log("Liste des destinataires:", nam_reseption_data_for_telecharger);
      console.log("Liste des totaux livrés:", somme_data_for_telecharger);
      console.log("Liste des dates:", time_data_for_telecharger);
      console.log("Liste des totaux affichés dans le logiciel :", sommeLogiciel_data_for_telecharger);
      console.log("Liste des totaux affichés dans la caisse :", sommeCaisse_data_for_telecharger);

      exporterMessageSentactualiserbtn.addEventListener('click', async () => {
       if(duredefiltrage===""){
         notification.classList.add('show');
         notification.style.backgroundColor="#e30000"
         notification.innerHTML = `Il faut choisir la date de fin de rapport au minimum pour l'imprimer.`;
         setTimeout(() => {
               notification.classList.remove('show');
               setTimeout(() => {
                 notification.style.backgroundColor="#289f00"
          ...
          notification.classList.add('show');
          notification.style.backgroundColor="#289f00";
          notification.innerHTML = `Telechargement en cours`;
     
          setTimeout(() => {
              notification.classList.remove('show');
          }, 5000);
     
         let nomberDeLine = nam_data_for_telecharger.length;
         console.log("le nombre de lignes :", nomberDeLine);
         exporterMessageSentactualiserbtnFunction(nomberDeLine);
       }

      });

      
      function exporterMessageSentactualiserbtnFunction(nomberDeLine) {
          let totalSomme = somme_data_for_telecharger.reduce((accumulateur, valeurCourante) => accumulateur + valeurCourante, 0);
          console.log("tot:", totalSomme);
          let totalSommelogiciel = sommeLogiciel_data_for_telecharger.reduce((accumulateur, valeurCourante) => accumulateur + valeurCourante, 0);
          console.log("tot:", totalSommelogiciel);
          let totalSommeDifferance = list_diff_log_encaisser.reduce((total, nombre) => total + nombre, 0);

          console.log("tot diff :", totalSommeDifferance);
          
         //  let totalSommeDifferance = totalSommelogiciel- totalSomme;

          const { jsPDF } = window.jspdf;
          const doc = new jsPDF();
          const logoFile = 'img/ALKOGAZ_LOGO.png'; // Path to the logo
          ...
          fetch(logoFile)
              ...
              .then(blob => {
                  ...
                  reader.onload = function(e) {
                      ...
                      doc.addImage(logoData, 'PNG', pageWidth - 50 - margin, margin, 45, 35); // Position and size of the logo
      
                      let billData = [];
      
                      for (let i = 0; i < nomberDeLine; i++) {
                          let data_i = {
                              nomDeenvo: nam_data_for_telecharger[i],
                              nomResepteur: nam_reseption_data_for_telecharger[i],
                              argentEencaiser: somme_data_for_telecharger[i],
                              argentAfficheParLOgiciel: sommeLogiciel_data_for_telecharger[i],
                              argentDansLaCaisse: sommeCaisse_data_for_telecharger[i],
                              temps: time_data_for_telecharger[i],
                          };
                          billData.push(data_i);
                      }
                      addInvoiceContent_toRapport(doc, duredefiltrage, billData,totalSommelogiciel,totalSomme,totalSommeDifferance);
                  };
                  reader.readAsDataURL(blob);
              });
      }
      
      function addInvoiceContent_toRapport(doc, duredefiltrage, billData,totalSommelogiciel,totalSomme,totalSommeDifferance) {
       // Title and header
       doc.setFontSize(12);
       doc.setFont('Helvetica', 'bold');
       doc.text('SARL ALCOGAZ', 10, 20);
       
       doc.setFontSize(9);
       doc.setFont('Helvetica', 'normal');
       doc.text(`Rapport de transaction d'argent`, 10, 25);
       doc.text(`La durée : ${duredefiltrage}`, 10, 30);  
   
       doc.setFontSize(12);
       doc.setFont('Helvetica', 'bold');
       doc.text('Liste Des Bordereaux D\'espèces Encaissées', 10, 35);
       doc.text('Service Commercial', 10, 43);
       ...
       doc.setFontSize(7);
       doc.setFont('Helvetica', 'bold');
       doc.text(10, tableStartY, 'Envoyé par');
       doc.text(45, tableStartY, 'Reçu par');
       doc.text(85, tableStartY, 'Encaissée');
       doc.text(105, tableStartY, 'logiciel');
       doc.text(125, tableStartY, 'Diff');
       doc.text(150, tableStartY, 'Caisse');
       doc.text(170, tableStartY, 'Date');
       doc.line(10, tableStartY + 5, 200, tableStartY + 5); // Horizontal line
   
       let currentY = tableStartY + 10;
   
       doc.setFontSize(8);
       doc.setFont('Helvetica', 'normal');
       billData.forEach(({ nomDeenvo, nomResepteur, argentEencaiser, argentAfficheParLOgiciel, argentDansLaCaisse, temps }) => {
           if (argentEencaiser > 0) {
               doc.text(10, currentY, nomDeenvo);
               doc.text(45, currentY, `${nomResepteur}`);
               doc.text(85, currentY, `${argentEencaiser}`);
               doc.text(105, currentY, `${argentAfficheParLOgiciel}`);
               if(argentAfficheParLOgiciel === 0){
                doc.text(125, currentY, `${0}`);
               }else{
                doc.text(125, currentY, `${(argentAfficheParLOgiciel - argentEencaiser).toFixed(0)}`);
               }
               doc.text(150, currentY, `${argentDansLaCaisse}`);
               doc.text(170, currentY, `${temps}`);
               currentY += 6;
           }
       });
   
       // Horizontal line above the total
       doc.line(10, currentY + 0, 200, currentY + 0);

       doc.setFontSize(8);
       doc.setFont('Helvetica', 'bold');
       doc.text(`${totalSommelogiciel.toFixed(1)}`, 105, currentY + 5);
       doc.text(`${totalSomme.toFixed(1)}`, 85, currentY + 5);
       doc.text(`${totalSommeDifferance.toFixed(1)}`, 125, currentY + 5);

       doc.line(10, currentY + 10, 200, currentY + 10);


       console.log("Téléchargement en cours");
       doc.save('rapport.pdf'); // Uncomment to save the PDF file
      }
     ...
     function createBordereauMESSAGESENT(nomuser, total_livree_envoyer, formattedDate,borderauid,archiveId,userid,statutsentborderau,motdepasss,envoyerVersID){
        ...
        async function remplacedatainoptions(sentversuserName){
         ...
         // nam_data_for_telecharger.push(nomuser);
         // nam_reseption_data_for_telecharger.push(sentversuserName);
         // somme_data_for_telecharger.push(total_livree_envoyer);
         // time_data_for_telecharger.push(formattedDate);

         // return { nam_data_for_telecharger, nam_reseption_data_for_telecharger, somme_data_for_telecharger, time_data_for_telecharger };

           
        }
        function viderListe(){
         nam_data_for_telecharger = [] ;
         somme_data_for_telecharger = [] ;
         sommeLogiciel_data_for_telecharger = [] ;
         time_data_for_telecharger = [] ;
         nam_reseption_data_for_telecharger = [] ;
         sommeCaisse_data_for_telecharger = [] ;
         list_diff_log_encaisser = [] ;

        return { nam_data_for_telecharger, nam_reseption_data_for_telecharger, somme_data_for_telecharger, time_data_for_telecharger, sommeCaisse_data_for_telecharger,sommeLogiciel_data_for_telecharger,list_diff_log_encaisser};
        ...
        btnValider.onclick = async function() {
         ...
         if(inputFin.value === ""){
           ...
           const datefin = new Date(dateFin);
           const formattedDatefin = `${datefin.getDate().toString().padStart(2, '0')} ${datefin.toLocaleString('default', { month: 'long' })} ${datefin.getFullYear()} ${datefin.getHours().toString().padStart(2, '0')}:${datefin.getMinutes().toString().padStart(2, '0')}`;
           duredefiltrage = "     de "+formattedDatedebut+" AM ,         au  "+ formattedDatefin +" AM ."
           filtrePar.textContent = "Filtre par :  "+duredefiltrage;
           ...
           draftCollectionQuery_.forEach((doc) => {
             ...
             const envoyerVersID = data.envoyerVersID;
               ...
               // Formatter la date au format "dd mm yyyy hh:mm"
               const formattedDate = `${date.getDate().toString().padStart(2, '0')} ${date.toLocaleString('default', { month: 'long' })} ${date.getFullYear()} ${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}`;
               recupererlenomderecepteur_pushdata(envoyerVersID,nomuser,total_livree_envoyer,formattedDate,archiveId);
           ...
           async function recupererlenomderecepteur_pushdata(envoyerVersID,nomuser,total_livree_envoyer,formattedDate,archiveId,duredefiltrage){
                   ...
                   if (userDocSnapshot.exists()) {
                       ...
                       somme_data_for_telecharger.push(total_livree_envoyer);
                       time_data_for_telecharger.push(formattedDate);
                       recuperedatapourPaport(archiveId,total_livree_envoyer);

                       return { nam_data_for_telecharger, nam_reseption_data_for_telecharger, somme_data_for_telecharger, time_data_for_telecharger,duredefiltrage };
             
                   } else {
                       console.log("Aucun document trouvé !");
                   }
            }
           async function recuperedatapourPaport(archiveId,total_livree_envoyer){
                 ...
                 if (docSnap.exists()) {
                     ...
                     sommeLogiciel_data_for_telecharger.push(tot_logiciel); 
                     sommeCaisse_data_for_telecharger.push(total_caisse); 
                     if(tot_logiciel===0){
                       const diff_lo_total_livree_envoyer_num = 0;
                       list_diff_log_encaisser.push(diff_lo_total_livree_envoyer_num); 
                      }else{
                        const diff_lo_total_livree_envoyer_num = tot_logiciel-total_livree_envoyer;
                        list_diff_log_encaisser.push(diff_lo_total_livree_envoyer_num); 
                      }
 
                      return { sommeCaisse_data_for_telecharger, sommeLogiciel_data_for_telecharger,list_diff_log_encaisser};
     ...
     dataShowBg.appendChild(exporterMessageSentactualiserbtn)     
      ...
      draftCollectionQuery_.forEach((doc) => {
          ...
          Annulerenvoyerbordo.onclick = async function() {
             ...
             function exporterMessageSentactualiserbtnFunction(nomberDeLine) {
                 let totalSomme = somme_data_for_telecharger.reduce((accumulateur, valeurCourante) => accumulateur + valeurCourante, 0);
                 console.log("tot encais:", totalSomme);
                 let totalSommelogiciel = sommeLogiciel_data_for_telecharger.reduce((accumulateur, valeurCourante) => accumulateur + valeurCourante, 0);
                 console.log("tot logi:", totalSommelogiciel);
                 let totalSommeDifferance = list_diff_log_encaisser.reduce((total, nombre) => total + nombre, 0);
                 console.log("tot diff :", totalSommeDifferance);
                         ...
                         reader.onload = function(e) {
                             ...
                             for (let i = 0; i < nomberDeLine; i++) {
                                 let data_i = {
                                     ...
                                     argentAfficheParLOgiciel: sommeLogiciel_data_for_telecharger[i],
                                     argentDansLaCaisse: sommeCaisse_data_for_telecharger[i],
                                     // diff_lo_encaisse: list_diff_log_encaisser[i],
                                     temps: time_data_for_telecharger[i],
                                 };
             ...
             function addInvoiceContent_toRapport(doc, duredefiltrage, billData,totalSommelogiciel,totalSomme,totalSommeDifferance) {
              ...
              doc.text(`${totalSomme.toFixed(1)}`, 85, currentY + 5);
              doc.text(`${totalSommeDifferance.toFixed(1)}`, 125, currentY + 5);
             //  doc.text(`${Caisse_Fin.toFixed(1)}`, 150, currentY + 5);
           ...
           console.log("statutsentborderau = ",statutsentborderau);
                  ...
                  if(inputFin.value === ""){
                    ...
                    draftCollectionQuery_.forEach((doc) => {
                        ...
                        // Formatter la date au format "dd mm yyyy hh:mm"
                        const formattedDate = `${date.getDate().toString().padStart(2, '0')} ${date.toLocaleString('default', { month: 'long' })} ${date.getFullYear()} ${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}`;
                        recupererlenomderecepteur_pushdata_valide(receptionnerpar,nomuser,total_livree_envoyer,formattedDate,archiveId,duredefiltrage);
                        // Ajouter un bordereau dans la liste avec la date formatée
                        createBordereauMessageValide(nomuser, total_livree_envoyer, formattedDate,borderauid,archiveId,userid,statutsentborderau,receptionnerpar);
                    ...
                    async function recupererlenomderecepteur_pushdata_valide(receptionnerpar,nomuser,total_livree_envoyer,formattedDate,archiveId,duredefiltrage){
       
                         nam_data_for_telecharger.push(nomuser);
                         nam_reseption_data_for_telecharger.push(receptionnerpar);
                         somme_data_for_telecharger.push(total_livree_envoyer);
                         time_data_for_telecharger.push(formattedDate);
                         recuperedatapourPaportvalide(archiveId,total_livree_envoyer);
 
                         return { nam_data_for_telecharger, nam_reseption_data_for_telecharger, somme_data_for_telecharger, time_data_for_telecharger,duredefiltrage };

                    }
                    async function recuperedatapourPaportvalide(archiveId,total_livree_envoyer){
                          ...
                          if (docSnap.exists()) {
                              ...
                              sommeLogiciel_data_for_telecharger.push(tot_logiciel); 
                              sommeCaisse_data_for_telecharger.push(total_caisse); 
                              console.log("total_caisse= "+total_caisse);
                              console.log("total_caisse= "+tot_logiciel);
   ...
   async function actualiserLaGestionDeStock(){
      wating_page.style.display="flex";
    ...
    // Création du conteneur principal
    const passwordAcceeBG = document.createElement("div");
    passwordAcceeBG.classList.add("passwordAcceeBG");
    
    // Titre "Filtrage"
    const passwordAcceeBG_titre = document.createElement("h1");
    passwordAcceeBG_titre.textContent = "GESTION DE STOCK DE GRAVIER ";
    passwordAcceeBG.appendChild(passwordAcceeBG_titre);

    const passwordAcceeBG_p = document.createElement("h3");
    passwordAcceeBG_p.textContent = "Entrez le mot de passe pour accéder à cette fonctionnalité";
    passwordAcceeBG.appendChild(passwordAcceeBG_p);

    const passwordAcceeBG_IMG = document.createElement("img");
    passwordAcceeBG_IMG.src = "img/undraw_forgot_password_re_hxwm.svg";
    passwordAcceeBG.appendChild(passwordAcceeBG_IMG);
   //  <img  src="img/undraw_forgot_password_re_hxwm.svg" alt="">



    dataShowBg.appendChild(passwordAcceeBG);
    wating_page.style.display="none";

    const docRefGS = doc(db, 'gestionDeStock', "parametresDeProduction");
    const docSnapGS = await getDoc(docRefGS);

    if (docSnapGS.exists()) {
        console.log("Document exists");
        const datas = docSnapGS.data();
        const MDP = datas.mdp;
        ...
        // Vérifier si l'utilisateur a entré quelque chose
         if (code === MDP) {
                 ...
                 const h1parametre = document.createElement('h1');
                 h1parametre.textContent="les parametres de calcule";
                 h1parametre.className="h1parametre";

                 const pparametre = document.createElement('p');
                 pparametre.textContent="les parametres de calcules , les pourcentage utilisé dans les calcules de production de chaque fraction de gravier ";
                 pparametre.className="pparametre";

                 

                 divstock.appendChild(h1parametre);
                 divstock.appendChild(pparametre);
                 datashowbg.appendChild(divstock);
